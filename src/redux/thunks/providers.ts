...
import type { LoginAction, ProvidersListResult } from "../../protocol";
...
export const loginProvider = createAsyncThunk<LoginAction, { provider: string; method?: string }, ThunkApiType>(
    "providers/login",
    (params) => webviewSendAndGet('providers/login', params),
);
...
export const loginProviderInput = createAsyncThunk<LoginAction, { provider: string; data: Record<string, string> }, ThunkApiType>(
    ...
    (params) => webviewSendAndGet('providers/loginInput', params),
);
